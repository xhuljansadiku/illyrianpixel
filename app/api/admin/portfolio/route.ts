...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      category_en: String(body.category_en ?? "").trim().slice(0, 120) || null,
      location_en: String(body.location_en ?? "").trim().slice(0, 120) || null,
      description_en: String(body.description_en ?? "").trim().slice(0, 2000) || null,
      result_en: String(body.result_en ?? "").trim().slice(0, 300) || null,
